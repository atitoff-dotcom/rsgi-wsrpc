...
Компактная детерминированная упаковка списков записей в табличный формат [fields, rows]
для устранения дублирования строковых ключей, экономии 50-70% трафика и разгрузки GC Python.
Является встроенным компонентом системного ядра rsgi-wsrpc.
"""
...
def pack_tabular(
    ...
    Для сырых SQL-кортежей (без создания промежуточных dict):
        pack_tabular(cursor.fetchall(), fields=["id", "title"])
    """
    if not items:
