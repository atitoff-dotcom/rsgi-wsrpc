...
2. [Архитектура: Ядро + Плагины + Приложение](#-архитектура-ядро--плагины--приложение)
   * [Архитектура «для чайников» с псевдографикой (docs_ru/architecture_for_beginners.md)](architecture_for_beginners.md)
...
7. [Официальные системные плагины (Plugins)](#-официальные-системные-плагины-plugins)
   ...
   * [Плагин умного реактивного кэша (smart_cache)](#4-плагин-умного-реактивного-кэша-pluginssmart_cache)
...
│                                                                         │
│   Знает обо всех компонентах: настраивает проект в коде (Code-First),
...
│                                                                         │
│   [ Плагин: DB ]        [ Плагин: Auth ]       [ Плагин: Files ]        │
│   Асинхронный ORM       Пользователи, JWT,     Потоковая 2PC-загрузка,  │
│   SQLite / PostgreSQL   роли и доступы         реестр и Nginx-раздача   │
│                                                                         │
│   [ Прикладные плагины: Форум, Биллинг, Уведомления, Аналитика... ]     │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ регистрируются в
┌────────────────────────────────────▼────────────────────────────────────┐
│                        3. СЕТЕВОЕ ЯДРО (Core)                           │
│                                                                         │
│   Чистый высокопроизводительный сокетный рантайм (Granian на Rust).     │
│   НЕ ЗНАЕТ о базах данных и доменных моделях приложения.                │
│   Отвечает за: WSRPC (JSON-RPC 2.0), мультиретурн, сессии, 2PC.         │
└─────────────────────────────────────────────────────────────────────────┘
```

### Главные правила архитектуры:
1. **Ядро автономно**: в папке `core/` нет ни одного импорта из прикладных модулей или баз данных.
2. **Плагины модульны**: каждый плагин изолирован и регистрирует свои методы в ядре через интерфейсы (`@rpc_method`, `@on_startup`, `session.register_on_close`).
3. **Приложение управляет составом**: если вам нужен микросервис очередей без базы данных — вы просто не подключаете плагин `db`. Если нужен полноценный портал — подключаете весь набор батареек.

---

### 📁 Рекомендуемая структура проекта (дерево каталогов)

Ниже представлена проверенная продакшеном структура, разделяющая сетевой движок (`core/`), официальные батарейки (`app/system/`) и ваши прикладные фичи (`app/<модули>/`):

```text
my_project/
├── core/                           # ⚡ СЕТЕВОЕ ЯДРО (RSGI + WSRPC)
│   ├── lib/
│   │   └── config.py               # Code-First конфигурация и configure()
...
Установка легковесного ядра (Granian RSGI, Orjson, WSRPC, Cryptography):
```bash
pip install rsgi-wsrpc
```

Или установка с поддержкой БД и асинхронных драйверов под ваш стек:
```bash
pip install "rsgi-wsrpc[sqlite]"      # Асинхронный SQLite (aiosqlite + SQLAlchemy 2.0)
pip install "rsgi-wsrpc[postgres]"    # Асинхронный PostgreSQL (asyncpg + SQLAlchemy 2.0)
pip install "rsgi-wsrpc[mysql]"       # Асинхронный MySQL (asyncmy + SQLAlchemy 2.0)
pip install "rsgi-wsrpc[full]"        # Все драйверы и плагины включены
```

---

### ⚡ Вариант A: Интерактивное демо Showcase (в 1 клик)

В репозитории доступен готовый интерактивный проект (`examples/showcase/`):
...
* **Возможности**:
  * Модель `User`, гибкая ролевая модель (динамические роли в БД `auth_role`, поддержка кастомных `UserRole`, superadmin bypass).
...
* **Как работает Two-Phase Commit**:
  1. Клиент запрашивает транзакцию загрузки: сервер выделяет уникальную хэш-папку `/tmp/app_uploads/<folder_hash>/`.
...
### 4. Плагин умного реактивного кэша (`plugins/smart_cache`)
* Подробная архитектура: см. **[docs_ru/smart_cache.md](smart_cache.md)** и **[RFC 0001](rfc/0001-smart-cache.md)**.
* **Принцип 0 мс отклика и Push-инвалидации**:
  * Экран открывается мгновенно из L1 RAM (или L2 IndexedDB) без сетевых задержек.
  * Сервер отслеживает мутации и через декоратор `@invalidates(tags=...)` автоматически пушит импульс `cache.invalidate` или точечный `cache.patch`.
  * Никакого слепого поллинга (polling) — сокет молчит до момента реального изменения данных.
  * Рукопожатие версий (`cache.sync_check`) при реконнекте после оффлайна загружает только реально изменившиеся данные.

---

### 5. Модульный каркас тестирования (`tests/`)
* Подробное руководство: см. **[docs_ru/testing.md](testing.md)**.
* **Client-Perspective Black-Box Тестирование**:
  * Тестирует бэкенд точно так же, как с ним взаимодействует реальный фронтенд-клиент (по WebSocket WSRPC и HTTP).
  * `PersonaManager`: предварительно авторизованные сессии (`admin`, `user`, `guest`) с автоматическим сидингом локальной базы данных и обходом RLS.
  * Тестирование стриминга (`stream: true`), перехвата push-нотификаций (`cache.invalidate`, `cache.patch`) и двухфазной загрузки файлов.
  * Встроенное нагрузочное и стресс-тестирование (`tests/suites/test_load.py`): замер RPS, латентности p50/p95/p99 и надежности fan-out рассылки.

---

### 6. Детерминированное табличное сжатие (`app/system/tabular.py`)
* Подробное руководство: см. **[docs_ru/tabular_compression.md](tabular_compression.md)** и **[RFC 0002](rfc/0002-packed-tabular-payloads.md)**.
* **Экономия 50–70% трафика и разгрузка сборщика мусора (GC)**:
  * Устраняет дублирование строковых ключей в ответах коллекций за счет разделения схемы колонок и матрицы строк (`$tabular: true`).
  * Полностью прозрачная распаковка на стороне фронтенда (`wsrpc.ts`) исключает необходимость ручной обработки в UI-компонентах.
  * Позволяет передавать сырые кортежи из курсора базы данных напрямую в сетевой сериализатор без промежуточных словарей `dict(row)`.
