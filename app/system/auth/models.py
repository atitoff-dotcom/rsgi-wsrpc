...
from sqlalchemy import String, Integer, Boolean, Text, ForeignKey, Table, Column, DateTime, func, JSON
from sqlalchemy.orm import Mapped, mapped_column, relationship

from app.system.db import Base
from app.system.auth.core import RowSecureModel
...
class RefreshToken(Base):
    """
    Модель для хранения токенов обновления (JWT).
    """
...
class User(RowSecureModel):
    ...
    def __str__(self):
        return f'Пользователь {self.first_name} [{self.id}]'
    ...
    def get_permissions(self):
        is_superadmin = any(role.id == 1 or role.name == "admin" for role in self.roles)
    ...
    def check_permission(self, model_name, perm_name: str = None) -> tuple[bool, bool]:
    ...
    def verify_password(self, password: str) -> bool:
        ...
        if self.password_hash.startswith("pbkdf2_sha256$"):
            ...
            # Fallback to simple SHA-256 (legacy format)
    ...
    def verify_raw_password(self, raw_password: str) -> bool:
        from core.logger import logger
        logger.warning("Метод verify_raw_password устарел, используйте verify_password")
    ...
    def _hash_password(cls, password: str, salt: bytes = None, iterations: int = None) -> str:
        ...
        if iterations is None:
            ...
            # We access the security block from settings
            try:
                iterations = settings.security.password_iterations
            except AttributeError:
                iterations = 10000 # default fallback
