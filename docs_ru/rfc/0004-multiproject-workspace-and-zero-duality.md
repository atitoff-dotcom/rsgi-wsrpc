...
4. [Целевая архитектура рабочей области (Umbrella Workspace)](#4-целевая-архитектура-рабочей-области-umbrella-workspace)
5. [Пакет бэкенда Python и канонический неймспейс (`rsgi_wsrpc`)](#5-пакет-бэкенда-python-и-канонический-неймспейс-rsgi_wsrpc)
6. [Интеграция фронтенд-клиента и алиасы Vite](#6-интеграция-фронтенд-клиента-и-алиасы-vite)
7. [Окружение, утилиты и развертывание на сервере](#7-окружение-утилиты-и-развертывание-на-сервере)
8. [Поэтапный план миграции](#8-поэтапный-план-миграции)
...
* **Единый источник правды для фронтенда**: TypeScript-клиент (`wsrpc.ts`, `smartCache.ts`) находится исключительно в репозитории `rsgi-wsrpc/client/` и подключается в приложения напрямую через алиасы Vite или npm-пакет.
...
## 4. Целевая архитектура рабочей области (Umbrella Workspace)
...
│   ├── docs/ & docs_ru/                    # Каноническая документация Ядра и RFC (0001, 0002, 0003, 0004)
...
└── apps/                                   # Каталог прикладных сервисов
    └── agrita/                             # РЕПОЗИТОРИЙ 3: Платформа Агрита (Git: agrita)
        ...
        │   ├── models/                     # Агрономические ORM-модели (удобрения, рецепты, культуры)
        │   ├── handlers/                   # WSRPC бизнес-методы (calc.*, forum.*)
...
## 5. Пакет бэкенда Python и канонический неймспейс (`rsgi_wsrpc`)

### 5.1 Описание пакета в `rsgi-wsrpc/pyproject.toml`
...
### 5.2 Канонический стиль импортов
...
### 5.3 Мост обратной совместимости (Zero-Breakage Bridge)
...
## 6. Интеграция фронтенд-клиента и алиасы Vite

### 6.1 Прямой алиас в `vite.config.ts`
Вместо ручного копирования файла `wsrpc.ts` фронтенд подключает клиент напрямую из репозитория фреймворка:
...
### 6.2 Прозрачное использование в компонентах
В любом компоненте Svelte или сторе:
```typescript
...
## 7. Окружение, утилиты и развертывание на сервере

### 7.1 Единое виртуальное окружение Python
Для экономии оперативной памяти и диска (особенно на серверах с 1 GB RAM):
* Создается единый `.venv` в корне зонтичной рабочей области.
* Фреймворк устанавливается в **режиме разработчика (Editable Mode)**:
  ```bash
  ...
  ```
* Любое изменение в файлах `rsgi-wsrpc/` немедленно подхватывается без переустановки.

### 7.2 Развертывание на сервере (Stage / Prod)
В скриптах деплоя (`deploy-stage.sh`):
...
graph TD
    ...
    Step2 --> Step3[3. Настройка Vite alias во фронтенде на client]
    Step3 --> Step4[4. Удаление дубликатов core/ и docs_core/ из Агриты]
    Step4 --> Step5[5. Обновление скриптов деплоя на Stage]
```
...
3. **Шаг 3: Настройка фронтенда**
   * Добавление алиаса `@wsrpc` в `apps/agrita/frontend/vite.config.ts`.
...
5. **Шаг 5: Верификация на Stage-сервере**
   * Развертывание новой раскладки на Stage (`195.133.5.32`).
...
Благодаря устранению дублирования, централизации документации фреймворка и стандартизации канонического пространства имен `rsgi_wsrpc` достигается абсолютная прозрачность кода, высокая скорость разработки и масштабируемость платформы под неограниченное число новых приложений.
